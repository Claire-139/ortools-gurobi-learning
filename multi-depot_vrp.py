...
# 可视化——节点坐标,仓库A和仓库B放在两端，客户节点散布在中间，这样路径图才好看
coordinates = [
    (10, 50),
    (90, 50),
    (20, 80),
...
time_windows = [(0, 480), (0, 480), (50, 260), (0, 150), (110, 410), (0, 480), (200, 450)]

# 每个节点的服务时间
service_times = [0, 0, 15, 20, 5, 30, 50]

# 多起点
starts = [0, 1]
ends = [0, 1]

# 创建路径模型：7个地点，2辆车，仓库在0,1号
# manager = pywrapcp.RoutingIndexManager(节点数, 车辆数, starts, ends)
manager = pywrapcp.RoutingIndexManager(7, 2, starts, ends)
...
def plot_routes(routes, coordinates):
    fig, ax = plt.subplots()
    # 1. 画出所有节点的点,区分仓库和客户用不同颜色,给每个节点标上编号
...
if solution:
    ...
    for vehicle_id in range(2): # 遍历车辆
        ...
        route.append(manager.IndexToNode(end_index))
